...
void searchByRegexStream(const std::string& filepath, const std::regex& pat, bool isContact = false) {
    std::ifstream f(filepath, std::ios::binary | std::ios::ate);
    if (!f) {
    ...
    std::cout << "[Press 's' to stop] Searching..." << std::endl;
    size_t count = 0;
        ...
        while (true) {
            ...
            if (choice == "1") {
                ...
                if (!fp.empty()) {
                    ...
                    searchByRegexStream(fp, rx, true);
                }
            }
            else if (choice == "3") {
                auto fp = BrowseForFile();
                if (!fp.empty()) {
                    ...
                    searchByRegexStream(fp, rx, false);
                }
            }
            else if (choice == "4") {
                break;
